...
def listenPeers():
	...
	while(True):
		...
		if(data1=='Backup'):
			...
			#print("lalalallalaalal not working ahead")
			data=conn.recv(1024)
			data=str(data,'utf-8')
			#print("me chl rha hun mjhe stupid mt bolo")
			print(data)
			#print("look at me!!!")
			fileName=data.split("#",1)[0]
			chunkData=data.split("#",1)[1]
			print(chunkData)
			#chunkData=bytes(chunkData,'utf-8')
			writeData=chunkData.split('\n')
			#print("hehehehehehehehehehehihihihihihih")
			with open('Backup_Store/'+ip+'/'+fileName, 'w') as f1:
